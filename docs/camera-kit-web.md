...
> Audience: any coding agent or developer working on this repo. Pairs with
> [`audio-capture.md`](./audio-capture.md), which covers the custom recording /
> audio-capture code that is **not** part of Camera Kit.
...
Camera Kit only renders to canvases — it does **not** record, and it provides no
API to capture lens audio as a stream. This repo adds recording on top of Camera
Kit using `canvas.captureStream()`, the Web Audio API, `MediaRecorder`, and
ffmpeg.wasm. Because that code is custom and easy to break, it has its own
document: see [`audio-capture.md`](./audio-capture.md).

The one Camera Kit detail that matters for recording is the render target: this
repo records `session.output.live` by default, or `session.output.capture` when
`Settings.recording.recordCaptureRenderTarget` is `true` (see Render targets
above).
